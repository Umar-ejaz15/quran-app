...
import { getRukuWithOffset } from '@/lib/api';
import { setCanonicalUrl, getRukuUrl } from '@/lib/canonical';
import { Ayah } from '@/types/quran';

export default function RukuDetailPage() {
  const params = useParams();
  const rukuNumber = parseInt(params.id as string);
  ...
  useEffect(() => {
    ...
    setCanonicalUrl(getRukuUrl(rukuNumber));

    async function loadRuku() {
      try {
        setLoading(true);
        const [arabicData, translationData] = await Promise.all([
          getRukuWithOffset(rukuNumber, reciterEdition),
          getRukuWithOffset(rukuNumber, selectedEdition),
        ]);
        setAyahs(arabicData.data.ayahs);
        setTranslations(translationData.data.ayahs);
      } catch (error) {
        console.error('Error loading ruku:', error);
      } finally {
        setLoading(false);
      }
    }
    loadRuku();
  }, [rukuNumber, selectedEdition, reciterEdition]);
