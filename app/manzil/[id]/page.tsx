...
import { setCanonicalUrl, getManzilUrl } from '@/lib/canonical';
...
export default function ManzilDetailPage() {
  ...
  useEffect(() => {
    ...
    setCanonicalUrl(getManzilUrl(manzilNumber));

    async function loadManzil() {
      try {
        setLoading(true);
        const [arabicData, translationData] = await Promise.all([
          getManzilWithOffset(manzilNumber, reciterEdition),
          getManzilWithOffset(manzilNumber, selectedEdition),
        ]);
    ...
    loadManzil();
  }, [manzilNumber, selectedEdition, reciterEdition]);
      ...
      <main className="container mx-auto px-4 md:px-6 py-6 md:py-10 max-w-4xl">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'var(--muted)' }}>{ayahs.length} verses</p>
              </div>
            </div>

            <div className="flex items-center gap-2">
              {manzilNumber > 1 && (
                <a href={`/manzil/${manzilNumber - 1}`} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold hover:-translate-y-0.5 transition-all" style={{ background: 'var(--hover)', border: '1px solid var(--border)', color: 'var(--foreground)' }}>
          ...
          <div className="mt-5 pt-5" style={{ borderTop: '1px solid var(--border)' }}>
            <button onClick={() => setShowSettings(v => !v)} className="flex items-center gap-2 text-xs font-semibold" style={{ color: 'var(--muted)' }}>
