...
export default function HizbDetailPage() {
  ...
  const hizbNumber = parseInt(params.id as string);
    ...
    async function loadHizb() {
      try {
        ...
        const [arabicData, translationData] = await Promise.all([
          getHizbQuarterWithOffset(hizbNumber, reciterEdition, offVal, limitVal),
          getHizbQuarterWithOffset(hizbNumber, selectedEdition, offVal, limitVal),
        ]);
        setAyahs(arabicData.data.ayahs);
        setTranslations(translationData.data.ayahs);
      } catch (error) {
        console.error('Error loading hizb quarter:', error);
      } finally {
        setLoading(false);
      }
    }
    loadHizb();
  }, [hizbNumber, selectedEdition, reciterEdition, offset, limit]);
